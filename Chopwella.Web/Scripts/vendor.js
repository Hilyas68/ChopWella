...
$(document).ready(function () {
    ...
    dataTable = $('#example1').DataTable({
        ...
        "ajax": {
            "url": "http://localhost:60532/api/chopwella/vendors",
            "type": "GET",
            "dataType": "json",
    ...
    $("#AddVendor").click(function (e) {
        ...
        formData.Name = $('#inputVendorName').val();


        $.ajax({
            url: 'http://localhost:60532/api/chopwella/addvendor',
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify(formData),
            success: function () {
                alert('Success');
                $('#AddModalVendor').modal('hide');
...
function Delete(id) {
    if (confirm("Are you sure you want to delete this staff?")) {
        $.ajax({
            url: "http://localhost:60532/api/chopwella/deleteVendor/" + id,
            method: "DELETE",
            success: function () {
